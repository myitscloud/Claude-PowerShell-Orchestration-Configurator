...
function Set-StartMenuSettings {
    ...
    try {
        ...
        Set-ItemProperty -Path $StartPath -Name "Start_TrackProgs" -Value $(if($ShowRecentlyAdded){1}else{0}) -PropertyType DWord -Force

        # Show most used apps
        Write-Log "Setting: Show Most Used = $ShowMostUsed" -Level "INFO"
        Set-ItemProperty -Path $StartPath -Name "Start_TrackDocs" -Value $(if($ShowMostUsed){1}else{0}) -PropertyType DWord -Force
        ...
        Set-ItemProperty -Path $StartContentPath -Name "Start_IrisRecommendations" -Value $(if($ShowRecommendations){1}else{0}) -PropertyType DWord -Force -ErrorAction SilentlyContinue
        ...
        switch ($StartMenuSize) {
            "MorePins" {
                Set-ItemProperty -Path $StartLayoutPath -Name "Start_Layout" -Value 0 -PropertyType DWord -Force -ErrorAction SilentlyContinue
            }
            "MoreRecommendations" {
                Set-ItemProperty -Path $StartLayoutPath -Name "Start_Layout" -Value 1 -PropertyType DWord -Force -ErrorAction SilentlyContinue
            }
            "Default" {
...
function Import-StartMenuLayout {
    ...
    try {
        ...
        Set-ItemProperty -Path $GPPath -Name "LockedStartLayout" -Value 1 -PropertyType DWord -Force
        Set-ItemProperty -Path $GPPath -Name "StartLayoutFile" -Value $LayoutDestination -PropertyType String -Force
