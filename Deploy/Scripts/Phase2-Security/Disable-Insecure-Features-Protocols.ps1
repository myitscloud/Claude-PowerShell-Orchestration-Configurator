...
function Disable-NetBIOSOverTCPIP {
    ...
    try {
        # Use Get-CimInstance for PowerShell 7+ compatibility
        # Get-WmiObject doesn't work properly in PowerShell 7
        if ($PSVersionTable.PSVersion.Major -ge 6) {
            # PowerShell 7+ - Use CimInstance
            $Adapters = Get-CimInstance -ClassName Win32_NetworkAdapterConfiguration | Where-Object { $_.IPEnabled }
        }
        else {
            # Windows PowerShell 5.1 - Use WMI
        ...
        foreach ($Adapter in $Adapters) {
            ...
            # Method call differs between PowerShell versions
            if ($PSVersionTable.PSVersion.Major -ge 6) {
                # PowerShell 7+ - Use Invoke-CimMethod
                $Result = Invoke-CimMethod -InputObject $Adapter -MethodName SetTcpipNetbios -Arguments @{ TcpipNetbiosOptions = 2 }
                $ReturnValue = if ($Result.ReturnValue -ne $null) { $Result.ReturnValue } else { 0 }
            }
            else {
                # Windows PowerShell 5.1 - Call method directly
            ...
            if ($ReturnValue -eq 0) {
                ...
                Write-Log "Failed to disable NetBIOS on: $AdapterName (Error: $ReturnValue)" -Level "ERROR"
                $Global:Stats.Errors++
            }
        }
    }
...
function Disable-IPv6Protocol {
    ...
    try {
        ...
        # Prefer IPv4 over IPv6 via registry (Microsoft-recommended method)
        $RegPath = "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip6\Parameters"
        if (-not (Test-Path $RegPath)) {
            New-Item -Path $RegPath -Force | Out-Null
        }

        # DisabledComponents = 0x20 prefers IPv4 but keeps IPv6 functional
        # This prevents DNS/networking issues while preferring IPv4
        # Microsoft explicitly warns against using 0xFF (full disable)
        ...
        Write-Log "IPv6 configured to prefer IPv4 (0x20) - DNS remains functional" -Level "SUCCESS"
        Write-Log "⚠️ REBOOT REQUIRED for IPv6 preference to take effect" -Level "WARNING"
        $Global:Stats.ProtocolsDisabled++
        $Global:Stats.RegistryChanges++
        $Global:Stats.RebootRequired = $true
    }
