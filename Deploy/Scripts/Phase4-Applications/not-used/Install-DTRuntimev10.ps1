...
Write-Host "    [Sub-Task] Starting Desktop Runtime v10" -ForegroundColor Gray
...
$InstallerFile = "windowsdesktop-runtime-10.0.0-win-x64.exe" 

# The full, expected path to the installer file
$InstallerPath = "Installers\DotNet\$InstallerFile"
...
if ($ExitCode -eq 0) {
    Write-Host "    [Sub-Task] DotNet Desktop Runtime 10 Installation Complete." -ForegroundColor Green
    Exit 0
} else {
    Write-Host "    [Sub-Task] DotNet Desktop Runtime 10 Installation FAILED." -ForegroundColor Red
    Exit 1
}