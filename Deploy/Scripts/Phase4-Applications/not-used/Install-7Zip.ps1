<#
.SYNOPSIS
    Installs 7Zip x64 silently via EXE.
.DESCRIPTION
...
Write-Host "    [Sub-Task] Starting 7ZIP x64 Installation." -ForegroundColor Gray
...
$InstallerFile = "7z2408-x64.msi"
$InstallerPath = "Installers\Apps\7Zip\$InstallerFile"
...
if ($ExitCode -eq 0) {
    ...
    try {
        ...
        if ($SuccessCodes -contains $FinalExitCode) {
            Write-Host "    [Success] 7Zip installation completed (Exit Code: $FinalExitCode)." -ForegroundColor Green
        } else {
...
if ($ExitCode -eq 0) {
    Write-Host "    [Sub-Task] 7Zip Installation Complete." -ForegroundColor Green
    Exit 0
} else {
    Write-Host "    [Sub-Task] 7Zip Installation FAILED." -ForegroundColor Red
    Exit 1
}